    ...
    const std::string& base_fn, size_t n_names, size_t this_name)
{
    ...
    */
    ...
    base_fn_fp.replace_extension(boost::filesystem::path(
        str(boost::format("%02d%s%07d%s") % this_name % "_" % n_names % base_fn_fp.extension().string())));
    ...
    std::vector<size_t> rx_channel_nums)
{
    ...
    for (size_t i = 0; i < buffs.size(); i++) {
        ...
        */
        const std::string this_filename = generate_out_filename(file, file_num, i);
    ...
    while (not stop_signal_called){
        ...
        if(num_total_samps >= 2000000){
          ...
          for (size_t i = 0; i < outfiles.size(); i++) {
            outfiles[i]->close();
            const std::string this_filename = generate_out_filename(file, file_num, i);
            outfiles[i] = std::shared_ptr<std::ofstream>(new std::ofstream(this_filename.c_str(), 
                                                                             std::ofstream::binary));
          }
          file_num += 1;
        }
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    std::cout << boost::format("SPB: %d ...") % spb << std::endl;
